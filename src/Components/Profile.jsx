...
const Profile = () => {
  ...
  const loggedInEmail = localStorage.getItem('username'); // stored during login
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const res = await axios.get(`http://localhost:3000/profile/${loggedInEmail}`);
        if (res.data) {
          setFormData(res.data);
          setProfileId(res.data._id);
        }
      } catch (err) {
        console.error("Profile not found. Create a new one.");
  ...
  const handleSaveOrUpdate = async () => {
    try {
      if (profileId) {
        ...
        alert(res.data || 'Profile created successfully');
      }
    } catch (err) {
      console.error(err);
      alert('Error saving/updating profile');
    }
  };
  ...
  return (
    <div className="profile-container">
      ...
      <div className="profile-form">
        ...
        <input type="email" name="email" placeholder="Email Address" value={formData.email} onChange={handleChange} readOnly />
        ...
        <div className="profile-buttons">
          <button onClick={handleSaveOrUpdate}>{profileId ? "Update" : "Save"}</button>
        </div>
      </div>
    </div>
  );
